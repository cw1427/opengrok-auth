...
author: sc.
"""
from flask import render_template, session, request, g, jsonify, redirect, flash
from app import appbuilder
from app.models import MyUser, UserExtInfo
import logging
from app.security_manager import login_required_api
...
import re
from app.utils import sse_auth_by_type
...
class ApiUserView(BaseView):
    ...
    @expose('/remotelogin', methods=['GET', 'POST'])
    def remote_login(self):
        """RESTAPI remote login action method."""
        if current_user.is_authenticated:
            sid = request.cookies.get(self.appbuilder.get_app.session_cookie_name)
            # check permission by project
            origin_url = request.headers.get('X-Original-URI')
            m = re.match('/([\w]+)/xref/([\w]+)', origin_url)
            if m:
                # match patten need authorization
                app_server = m.group(1)
                app_proj = m.group(2)
                log.debug("Need auth user={0}, server={1}, proj={2}".format(current_user.username, app_server,
                                                                            app_proj))
                app_auth_type = self.appbuilder.get_app.config.get('SSE_APP_AUTH_TYPE').get(app_server)
                if app_auth_type:
                    if not sse_auth_by_type(app_server, app_proj, current_user.username, app_auth_type):
                        return jsonify({'message': "need auth"}), 403
                return jsonify({'message': "success"})
            else:
                return jsonify({'message': "success"})
        if request.method == 'GET':
            # no authenticated return 401
            return make_response(jsonify({'msg': 'userName or password parameters error'}), 401)
        form = LoginForm_db()
        # check remote login origin url from cookie
        ...
        if form.validate_on_submit():
            ...
            if not user:
                ...
                return redirect(parse.urljoin(remote_base_url, 'login'))
            login_user(user, remember=False)
            return redirect(remote_from)
        return redirect(parse.urljoin(remote_base_url, 'login'))
